...
query = (
    races_ref
    .where(firestore.FieldPath.document_id(), ">=", date_prefix) # <-- Using firestore.FieldPath
    .where(firestore.FieldPath.document_id(), "<", date_prefix + "\uf8ff") # <-- Using firestore.FieldPath
)
...
if docs_found == 0:
    logger.warning(f"No race documents found for {date_prefix}. The pipeline is likely not populating data.")
elif tickets_found == 0:
    logger.warning(f"No race documents found with tickets for {date_prefix}. Check ticket generation/saving logic.")
else:
    logger.info(f"Data found for {date_prefix}. Issue might be in API filtering or front-end display.")
