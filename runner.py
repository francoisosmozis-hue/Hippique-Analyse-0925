...
def run_course(
    ...
    if rc != 0:
        logger.error(
            "analyse_courses_du_jour_enrichie failed",
            correlation_id=correlation_id,
            returncode=rc,
            stderr=stderr[:500]
        )
    ...
    if rc != 0:
        logger.warning("p_finale_export failed (non-blocking)", returncode=rc)
    ...
    if rc != 0:
        logger.warning("simulate_ev failed (non-blocking)", returncode=rc)
    ...
    if rc != 0:
        logger.warning("update_excel_with_results failed (non-blocking)", returncode=rc)
...
def run_post_race_results(date: str) -> dict[str, Any]:
    ...
    if rc != 0:
        logger.error("get_arrivee_geny failed", returncode=rc, stderr=stderr[:500])
    ...
    if rc != 0:
        logger.warning("update_excel_with_results failed", returncode=rc)
