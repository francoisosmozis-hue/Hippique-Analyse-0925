...
def main():
    parser = argparse.ArgumentParser(description="Génère un plan.json de test pour développement")
    parser.add_argument(
        "--date", default=None, help="Date au format YYYY-MM-DD (défaut: aujourd'hui)"
    )
    parser.add_argument(
        "--races", type=int, default=10, help="Nombre de courses à générer (défaut: 10)"
    )
    parser.add_argument(
        "--output", default="plan_test.json", help="Fichier de sortie (défaut: plan_test.json)"
    )

    args = parser.parse_args()

    print("🐴 Génération d'un plan de test...")
    ...
    print("   1. Copier dans le conteneur Docker")
    print("   2. Ou utiliser en entrée de test")
