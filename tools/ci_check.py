...
SKIP_DIRS = {
    ...
    "__pycache__","Hippique-Analyse-0925"
}
DEFAULT_TIMEOUT = 5
...
def has_argparse(p: Path) -> bool:
    try:
        ...
        return False
    return ("argparse.ArgumentParser(" in txt) or ("from argparse import ArgumentParser" in txt)
...
"""
    try:
        proc = subprocess.run([sys.executable, "-c", code], env=env, capture_output=True, text=True, timeout=timeout)
        return proc.returncode, (proc.stdout + proc.stderr)
    except subprocess.TimeoutExpired:
        return 124, f"Timeout ({timeout}s)"

def main():
    ap = argparse.ArgumentParser(description="CI checks without running business logic.")
    ...
    if failures:
        ...
        for path, why, msg in failures:
            print("\n---", path, ":", why)
            print(textwrap.shorten(msg.replace("\n"," | "), width=1000, placeholder="..."))
        sys.exit(1)
    else:
