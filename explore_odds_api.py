...
def explore_sports(api_key):
    ...
    try:
        ...
        print(
            "Remaining requests for this month:",
        ...
        for sport in sports:
            if (
                "horse" in sport.get("title", "").lower()
                or "racing" in sport.get("title", "").lower()
            ):
        ...
        if horse_racing_sport:
            ...
            print("\n---")
            print(
                "Could not automatically find a sport key for horse racing in the list."
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Explore The Odds API for available sports."
    )
    parser.add_argument(
        "--api-key", required=True, help="Your API key for The Odds API."
    )
