...
def load_and_filter(paths, month: str) -> pd.DataFrame:
    ...
    for p in paths:
        ...
        if df['date'].isna().all():
            ...
            if inferred:
                df['date'] = inferred
        mask = pd.to_datetime(df['date'], errors='coerce').dt.year.eq(y) & pd.to_datetime(
...
def summarize_month(all_df: pd.DataFrame):
    ...
    if base['jockey'].notna().any():
        j_sum = base.groupby('jockey', dropna=True, as_index=False).agg(
            starts=('jockey', 'size'),
        ...
        j_sum = pd.DataFrame(
            columns=['jockey', 'starts', 'mean_j_rate', 'median_j_rate', 'sd_j_rate']
        )

    # Aggregation by trainer
    if base['entraineur'].notna().any():
        e_sum = base.groupby('entraineur', dropna=True, as_index=False).agg(
            starts=('entraineur', 'size'),
        ...
        e_sum = pd.DataFrame(
            columns=['entraineur', 'starts', 'mean_e_rate', 'median_e_rate', 'sd_e_rate']
        )
...
def main():
    ap = argparse.ArgumentParser(
        description="Concatène tous les *_je.csv d'un mois et produit des résumés J/E."
    )
    ...
    if not paths:
        ...
        candidates = [
            ...
            if p.name.lower() in ("je.csv",) or p.name.lower().endswith(("_je_h5.csv", "_JE.csv"))
        ]
        paths = candidates
