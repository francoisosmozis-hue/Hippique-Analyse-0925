...
def _load_snapshot(snapshot_path: str) -> list[Mapping[str, Any]]:
...
def enrich_from_snapshot(snapshot_path: str, out_dir: str) -> dict:
    ...
    runners = _load_snapshot(snapshot_path)
    if not runners:
        return result

    normalised = list(_normalise_runners(runners, Path(snapshot_path)))
