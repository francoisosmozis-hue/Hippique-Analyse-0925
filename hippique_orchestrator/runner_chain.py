...
from hippique_orchestrator.scraping.online_fetch_boturfers import fetch_boturfers_race_details as ofz
from hippique_orchestrator.gcs_utils import disabled_reason, is_gcs_enabled, upload_file
from hippique_orchestrator.simulate_ev import simulate_ev_batch
from hippique_orchestrator.simulate_wrapper import PAYOUT_CALIBRATION_PATH
from hippique_orchestrator.validator_ev import ValidationError, validate_ev
...
def _normalize_boturfers_snapshot(payload: dict[str, Any], reunion: str, course: str) -> dict[str, Any]:
    """
    Normalise le snapshot brut scrapé de Boturfers en un format standardisé.
    """
    ...
    # Création des mappings id -> nom et id -> cote (cote non disponible directement ici)
    id2name = {str(r["num"]): r["nom"] for r in runners if "num" in r and "nom" in r}
    # Boturfers race details do not provide odds directly, they are fetched separately or later.
    # For now, we'll leave odds empty or handle them in a later stage.
    odds = {} 

    return {
        "rc": f"{reunion}{course}",
        "hippodrome": payload.get("hippodrome"), # Assuming this can be parsed from URL or elsewhere if needed
        "discipline": payload.get("discipline"), # Assuming this can be parsed from URL or elsewhere if needed
        "date": datetime.now().strftime('%Y-%m-%d'), # Boturfers race details don't have date
        "runners": runners,
        "id2name": id2name,
        "odds": odds,
    }
...
def _write_snapshot(
    ...
    *,
    ...
    if not url:
        ...
        boturfers_url_template = sources.get("boturfers", {}).get("race_url_template")
        if boturfers_url_template:
            url = boturfers_url_template.format(reunion=payload.reunion, course=payload.course)

    try:
        snapshot = ofz.fetch_boturfers_race_details(url=url)
        ...
        normalized_snapshot = _normalize_boturfers_snapshot(snapshot, payload.reunion, payload.course)
        payload_out = {
            "status": "ok",
            "rc": race_id,
            "phase": window,
