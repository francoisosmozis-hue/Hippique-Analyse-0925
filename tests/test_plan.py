...
@pytest.mark.asyncio
@patch("hippique_orchestrator.plan.programme_provider.get_programme", new_callable=AsyncMock)
async def test_build_plan_nominal_case(mock_get_programme):
    """
    Tests the happy path: the scraper returns valid data, and the plan is built and sorted correctly.
    """
    # Arrange
    mock_get_programme.return_value = MOCK_PROGRAMME_DATA
    ...
    mock_get_programme.assert_called_once_with(date.fromisoformat(test_date))
...
@pytest.mark.asyncio
@patch("hippique_orchestrator.plan.programme_provider.get_programme", new_callable=AsyncMock)
async def test_build_plan_scraper_fails(mock_get_programme, caplog):
    """
    Tests that an empty plan is returned if the scraper fails (returns None).
    """
    # Arrange
    mock_get_programme.return_value = None
    ...
    # Assert
    assert result_plan == []
    assert "Failed to fetch programme for 2025-12-20 from any source." in caplog.text


@pytest.mark.asyncio
@patch("hippique_orchestrator.plan.programme_provider.get_programme", new_callable=AsyncMock)
async def test_build_plan_handles_malformed_data(mock_get_programme, caplog):
    """
    Tests that entries with malformed RC format or missing start_time are skipped.
    """
    # Arrange
    mock_get_programme.return_value = MOCK_MALFORMED_DATA
...
@pytest.mark.asyncio
@patch("hippique_orchestrator.plan.programme_provider.get_programme", new_callable=AsyncMock)
async def test_build_plan_today_string(mock_get_programme):
    """
    Tests that passing "today" uses the correct URL for the current day's programme.
    """
    mock_get_programme.return_value = []

    await plan.build_plan_async("today")

    mock_get_programme.assert_called_once()


@pytest.mark.asyncio
@patch("hippique_orchestrator.plan.programme_provider.get_programme", new_callable=AsyncMock)
async def test_build_plan_empty_enriched_plan(mock_get_programme, caplog):
    """
...
@pytest.mark.asyncio
@patch("hippique_orchestrator.plan.programme_provider.get_programme", new_callable=AsyncMock)
async def test_build_plan_partants_non_digit_string(mock_get_programme):
    """
...
@pytest.mark.asyncio
@patch("hippique_orchestrator.plan.programme_provider.get_programme", new_callable=AsyncMock)
async def test_build_plan_handles_compact_rc_format(mock_get_programme):
    """
