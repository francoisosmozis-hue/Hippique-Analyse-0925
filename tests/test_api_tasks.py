...
def mock_dependencies():
    ...
    with (
        patch("hippique_orchestrator.api.tasks.write_snapshot_for_day", new_callable=AsyncMock) as mock_write_snapshot,
...
async def test_snapshot_9h_task_success(mock_dependencies, mock_get_correlation_id):
    ...
    response = client.post("/tasks/snapshot-9h", json={})
...
async def test_snapshot_9h_task_with_specific_date_and_urls(mock_dependencies, mock_get_correlation_id):
    ...
    response = client.post("/tasks/snapshot-9h", json={"date": test_date, "meeting_urls": test_urls})
    assert response.status_code == 202
    assert response.json()["ok"] is True
...
async def test_run_phase_task_success(mock_dependencies, mock_get_correlation_id, mocker):
    ...
    mock_run_analysis_for_phase.return_value = {"gpi_decision": "PLAY", "status": "success", "tickets_analysis": []}
    ...
    assert response.json()["document_id"] == test_doc_id
    assert response.json()["gpi_decision"] == "PLAY"
    
    mock_run_analysis_for_phase.assert_called_once_with(
        course_url=test_course_url,
        phase=test_phase,
        date=test_date,
        race_doc_id=test_doc_id,
        correlation_id="12345678-1234-5678-1234-567812345678",
    )
...
async def test_run_phase_task_run_course_fails(mock_dependencies, mock_get_correlation_id, mocker):
    ...
    assert f"Failed to process task for {test_doc_id}. See logs for details." in response.json()["detail"]
...
async def test_bootstrap_day_task_success(mock_dependencies, mock_get_correlation_id):
    ...
    response = client.post("/tasks/bootstrap-day", json={})
    assert response.status_code == 202
    assert response.json()["ok"] is True
...
async def test_bootstrap_day_task_empty_plan(mock_dependencies, mock_get_correlation_id):
    ...
    mock_dependencies["mock_enqueue_run_task"].assert_not_called()

@pytest.mark.asyncio
async def test_bootstrap_day_task_exception_handling(mock_dependencies, mock_get_correlation_id):
    """
    Test bootstrap-day task when an unexpected exception occurs.
    """
    mock_build_plan = mock_dependencies["mock_build_plan"]
    mock_build_plan.side_effect = Exception("plan_build_error")

    response = client.post("/tasks/bootstrap-day", json={})
    assert response.status_code == 500
    assert "Internal server error during bootstrap: plan_build_error" in response.json()["detail"]
