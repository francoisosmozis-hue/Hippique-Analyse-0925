...
def mock_gpi_config():
    ...
    base_config = {
        ...
        "adjustments": {
            "chrono": {"k_c": 0.18},
            "drift": {
                "k_d": 0.70, "threshold": 0.07,
                "favorite_odds": 4.0, "favorite_factor": 1.20, "k_d_fav_drift": 0.90,
                "outsider_odds": 8.0, "outsider_steam_factor": 0.85, "k_d_out_steam": 0.85,
                "high_odds": 60.0, "extreme_drift_factor": 1.80,
            },
...
def test_initialize_and_validate_success(mock_snapshot_data, mock_gpi_config):
    ...
    raw_gpi_config = {
        k: v for k, v in mock_gpi_config.items()
...
def test_initialize_and_validate_missing_budget_raises_error(mock_snapshot_data, mock_gpi_config):
    ...
    with pytest.raises(ValueError, match="Configuration file is missing 'budget' or 'budget_cap_eur'"):
...
def test_initialize_and_validate_missing_critical_key_raises_error(mock_snapshot_data, mock_gpi_config):
    raw_gpi_config = {
        k: v for k, v in mock_gpi_config.items()
        if k not in ["chrono_config", "drift_config", "sp_config", "exotics_config"]
    }
...
def test_calculate_adjusted_probabilities_missing_base_probabilities_raises_error(mock_snapshot_data, mock_gpi_config):
...
def test_calculate_adjusted_probabilities_p_base_fallback_from_win_odds(mock_snapshot_data, mock_gpi_config):
    ...
    runners_adjusted, messages = _calculate_adjusted_probabilities(mock_snapshot_data["runners"], mock_gpi_config)
...
def test_calculate_adjusted_probabilities_real_favorites_detection(mock_snapshot_data, mock_gpi_config):
...
def test_calculate_adjusted_probabilities_overround_calculation(mock_snapshot_data, mock_gpi_config):
    runners_adjusted, messages = _calculate_adjusted_probabilities(mock_snapshot_data["runners"], mock_gpi_config)
    assert "overround_win" in mock_gpi_config["market"]
...
def test_apply_chrono_adjustment_success(mock_snapshot_data, mock_gpi_config):
    ...
    factors = _apply_chrono_adjustment(runners_for_test, je_stats, mock_gpi_config["adjustments"]["chrono"])
...
def test_apply_drift_adjustment_stable_odds(mock_snapshot_data, mock_gpi_config):
    ...
    factors = _apply_drift_adjustment(runners[:2], h30_odds_map, mock_gpi_config["adjustments"]["drift"])
    assert all(f == 1.0 for f in factors)
...
def test_generate_sp_dutching_tickets_profitable_candidates_creates_ticket(mock_snapshot_data, mock_gpi_config):
    runners = mock_snapshot_data["runners"]
...
@pytest.mark.parametrize("exotic_type, expected_legs", [
    ("COUPLE", 2),
    ("COUPLE_PLACE", 2),
    ("ZE234", 2),
    ("TRIO", 3),
...
def test_generate_exotic_tickets_no_profitable_combos(mocker: MockerFixture, mock_snapshot_data, mock_gpi_config):
...
def test_generate_tickets_populates_top5_pronostic(mock_snapshot_data, mock_gpi_config, mocker: MockerFixture):
    # Mock internal calls to simplify and control flow
    mocker.patch("hippique_orchestrator.pipeline_run._initialize_and_validate", return_value=(mock_snapshot_data["runners"], mock_gpi_config))
    mocker.patch("hippique_orchestrator.pipeline_run._calculate_adjusted_probabilities", return_value=(mock_snapshot_data["runners"], []))
    mocker.patch("hippique_orchestrator.pipeline_run._generate_sp_dutching_tickets", return_value=(mock_snapshot_data["runners"], [], []))
    mocker.patch("hippique_orchestrator.pipeline_run._generate_exotic_tickets", return_value=([], []))
    mocker.patch("hippique_orchestrator.pipeline_run._finalize_and_decide", return_value={
        "gpi_decision": "Play", "tickets": [], "roi_global_est": 0.3, "message": ""
    })
...
def test_generate_tickets_populates_market_analysis_table(mock_snapshot_data, mock_gpi_config, mocker: MockerFixture):
    mocker.patch("hippique_orchestrator.pipeline_run._initialize_and_validate", return_value=(mock_snapshot_data["runners"], mock_gpi_config))
    mocker.patch("hippique_orchestrator.pipeline_run._calculate_adjusted_probabilities", return_value=(mock_snapshot_data["runners"], []))
    mocker.patch("hippique_orchestrator.pipeline_run._generate_sp_dutching_tickets", return_value=(mock_snapshot_data["runners"], [], []))
    mocker.patch("hippique_orchestrator.pipeline_run._generate_exotic_tickets", return_value=([], []))
