...
def mock_gpi_config() -> dict:
    ...
    return {
        "budget": 5.0,
        "calibration_data": {"exotic_weights": {"TRIO": 1.0}},
        "je_stats": {},
        ...
        "adjustments": {"chrono": {"k_c": 0.18}, "drift": {"k_d": 0.70}},
        "tickets": {
            "sp_dutching": {
                "budget_ratio": 0.6,
                "legs_min": 2,
...
def test_generate_tickets_creates_trio_ticket_with_best_roi(
    ...
    def evaluate_combo_side_effect(tickets, bankroll, calibration):
        ...
        if horse_nums == {1, 2, 3}:
            return {"status": "ok", "roi": 0.8, "payout_expected": 50.0}
        if horse_nums == {1, 2, 4}:
            return {"status": "ok", "roi": 0.5, "payout_expected": 30.0}
        return {"status": "ok", "roi": 0.1, "payout_expected": 5.0}

    mocker.patch(
        "hippique_orchestrator.pipeline_run.evaluate_combo",
        side_effect=evaluate_combo_side_effect,
    )
    ...
    assert trio_ticket["stake"] == mock_gpi_config["budget"] * (
        1 - mock_gpi_config["tickets"]["sp_dutching"]["budget_ratio"]
    )
