...
def test_fetch_boturfers_race_details_success(mocker):
    ...
    mocker.patch(
        "hippique_orchestrator.scrapers.boturfers.fetch_boturfers_race_details",
        return_value={"runners": ["test"]},
    )
...
def test_fetch_boturfers_race_details_fails(mocker):
    ...
    mocker.patch(
        ...
        side_effect=HTTPError("Mocked HTTP Error"),
    )
    ...
    logger_mock.error.assert_has_calls(
        [
            call(
                "Erreur lors du téléchargement de http://dummy.url/partant: Mocked HTTP Error",
