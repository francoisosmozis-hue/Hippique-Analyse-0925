...
def test_fetch_boturfers_race_details_success(mocker):
    ...
    online_fetch_boturfers.BoturfersFetcher.assert_called_once_with(race_url="http://dummy.url")
...
def test_fetch_boturfers_race_details_fails(mocker):
    ...
    assert result == {}
    logger_mock.error.assert_called_with("Le scraping des détails a échoué pour http://dummy.url.")
