...
def fake_fs_cron_decider(fs: FakeFilesystem):
    ...
    fs.create_file("scripts/runner_chain.py", contents="print('runner_chain executed')")
    yield fs
...
def test_load_meetings_from_dict_key_meetings(fake_fs_cron_decider: FakeFilesystem):
    ...
    fake_fs_cron_decider.create_file("meetings.json", contents=json.dumps(meetings_data))
...
@patch(
    "hippique_orchestrator.scripts.cron_decider.os.environ", new_callable=lambda: os.environ.copy()
)
...
def test_main_happy_path_h5(mock_invoke_runner, mock_dt, fake_fs_cron_decider: FakeFilesystem):
...
@patch("hippique_orchestrator.scripts.cron_decider.dt")
@patch("hippique_orchestrator.scripts.cron_decider._invoke_runner")
def test_main_happy_path_h30(mock_invoke_runner, mock_dt, fake_fs_cron_decider: FakeFilesystem):
