...
def test_api_pronostics_default_date_is_today(client, mocker, mock_firestore_client):
    ...
    mocker.patch("hippique_orchestrator.service.datetime", MagicMock(now=MagicMock(return_value=mock_today)))
...
def test_api_pronostics_etag_304_not_modified(client, mock_firestore_client, mocker):
    ...
    mocker.patch("hippique_orchestrator.service.datetime", MagicMock(now=MagicMock(return_value=mock_now)))
...
def test_api_pronostics_rich_response_structure(client, mocker):
    ...
    docs = [
        MockDocumentSnapshot("2025-01-01_R1C1", {
            "rc": "R1C1", "last_analyzed_at": now_iso,
            "tickets_analysis": {"gpi_decision": "Play"}
        }),
        MockDocumentSnapshot("2025-01-01_R1C2", {
            "rc": "R1C2", "last_analyzed_at": now_iso,
            "tickets_analysis": {"gpi_decision": "Abstain"}
        }),
        MockDocumentSnapshot("2025-01-01_R1C3", {
            "rc": "R1C3", "last_analyzed_at": now_iso,
            "tickets_analysis": {"gpi_decision": "ERROR"}
        }),
    ]
    # We have 3 docs in DB, but let's say the plan has 4 races.
    plan_races = [
        {"rc_label": "R1C1"}, {"rc_label": "R1C2"}, {"rc_label": "R1C3"}, {"rc_label": "R1C4"}
    ]
    ...
    expected_counts_keys = {
        "total_in_plan", "total_processed", "total_analyzed",
        "total_playable", "total_abstain", "total_error", "total_pending"
    }
