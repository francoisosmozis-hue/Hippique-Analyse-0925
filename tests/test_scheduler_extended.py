...
def test_enqueue_run_task_no_service_url(
    mock_auth_default, mock_cloud_tasks, caplog: LogCaptureFixture
):
    """
...
def test_enqueue_run_task_permission_denied(
    ...
    """
    mock_auth_default.return_value = (None, "test-project")
    mock_cloud_tasks.create_task.side_effect = gexc.PermissionDenied("Forbidden by policy")
...
def test_enqueue_run_task_generic_exception(
    ...
    """
    mock_auth_default.return_value = (None, "test-project")
    mock_cloud_tasks.create_task.side_effect = Exception("Network error")
...
def test_schedule_all_races_real_run_no_service_url(
    mock_enqueue_run_task, caplog: LogCaptureFixture
):
    """
...
@patch("hippique_orchestrator.scheduler.enqueue_run_task")
def test_schedule_all_races_results_are_sorted(mock_enqueue_run_task):
    """
