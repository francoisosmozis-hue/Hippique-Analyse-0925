...
def test_allocation_plan_serialization():
    ...
    plan = AllocationPlan(
        allocations=[allocation], bankroll=100.0, expected_return=5.0, aggregate_risk=0.1
    )
...
def test_extract_metrics_nominal():
    payload = {
        "metrics": {"total_stake": 100, "roi": 0.2, "ev": 20, "risk_of_ruin": 0.1, "clv": 0.5}
    }
...
def test_extract_metrics_ev_block():
    payload = {"metrics": {"ev": {"roi": 0.25, "ev": 25, "risk_of_ruin": 0.15}}, "budget": 100}
    stake, ev, roi, risk, clv = _extract_metrics(payload)
    assert stake == 100
    assert ev == 25
    assert roi == 0.25
...
def test_extract_metrics_calculates_ev_from_roi_and_stake():
    payload = {"metrics": {"total_stake": 50, "roi": 0.3}}
    stake, ev, roi, risk, clv = _extract_metrics(payload)
    assert stake == 50
...
def analysis_files(tmp_path):
    ...
    p1 = d / "race1"
    p1.mkdir()
    (p1 / "analysis.json").write_text(
        json.dumps(
            {
                "race_id": "R1C1",
                "metrics": {"total_stake": 10, "ev": 1, "roi": 0.1, "risk_of_ruin": 0.05},
            }
        )
    )
    p2 = d / "race2"
    p2.mkdir()
    (p2 / "analysis.json").write_text(
        json.dumps(
            {
                "race_id": "R1C2",
                "metrics": {"total_stake": 20, "ev": 5, "roi": 0.25, "risk_of_ruin": 0.1},
            }
        )
    )
    # Malformed file
...
def test_score_high_risk():
    race = RaceMetrics(
        race_id="r1", stake=10, ev=1, roi=0.1, risk_of_ruin=0.01
    )  # Lower initial risk
    assert _score(race, target_ror=0.05) > _score(
        race, target_ror=0.1
    )  # Higher target_ror should lead to lower score (higher risk)
...
def test_scale_risk_zero_stake():
    ...
    assert (
        _scale_risk(race, target_stake=20) == race.safe_risk
    )  # Returns safe_risk if original stake is zero
