...
def test_fallback_parse_html_extracts_data(zeturf_course_html):
    ...
    assert runner_1["cote"] == 6.5
    assert runner_1["odds_place"] == 2.25

    runner_12 = next((r for r in runners if r.get("num") == "12"), None)
    assert runner_12 is not None
    assert runner_12["name"] == "HACKER JULRY"
    assert runner_12["cote"] == 25.4
    assert runner_12["odds_place"] == 5.2
