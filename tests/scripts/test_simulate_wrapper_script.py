...
def test_correlation_penalty_defaults_gracefully(fs, clear_caches):
    ...
    with patch(
        "hippique_orchestrator.scripts.simulate_wrapper.PAYOUT_CALIBRATION_PATH", payout_calib_path
    ):
        prob = simulate_wrapper(legs)
...
def test_monte_carlo_is_skipped_when_numpy_missing(fs, clear_caches):
    ...
    with patch(
        ...
        with patch(
            "hippique_orchestrator.scripts.simulate_wrapper.np", None
...
def test_evaluate_combo_calls_dependencies_and_returns_results(fs, clear_caches):
    ...
    with patch(
        "hippique_orchestrator.scripts.simulate_wrapper.compute_ev_roi", return_value=mock_stats
    ) as mock_compute_ev:
        with patch(
            "hippique_orchestrator.scripts.simulate_wrapper.simulate_wrapper", return_value=0.1
        ) as mock_simulate:
