...
def test_gate_ev_sp_fails(base_cfg):
    gates = gate_ev(cfg=base_cfg, ev_sp=0.1, roi_sp=0.01, ev_global=20, roi_global=0.2, min_payout_combos=15, ev_over_std=0.6)
...
def test_gate_ev_combo_fails(base_cfg):
    gates = gate_ev(cfg=base_cfg, ev_sp=10, roi_sp=0.2, ev_global=1, roi_global=0.05, min_payout_combos=5, ev_over_std=0.6)
...
def test_gate_ev_risk_fails_both(base_cfg):
    gates = gate_ev(cfg=base_cfg, ev_sp=10, roi_sp=0.2, ev_global=20, roi_global=0.2, min_payout_combos=15, risk_of_ruin=0.3, ev_over_std=0.6)
    assert gates["sp"] is False
    assert gates["combo"] is False
...
def test_gate_ev_sharpe_fails_both(base_cfg):
    gates = gate_ev(cfg=base_cfg, ev_sp=10, roi_sp=0.2, ev_global=20, roi_global=0.2, min_payout_combos=15, risk_of_ruin=0.1, ev_over_std=0.1)
    assert gates["sp"] is False
    assert gates["combo"] is False
