...
def run_hminus(payload: Dict = Body(...)):
    ...
    if not R or not C:
        raise HTTPException(status_code=400, detail="Body JSON requis: R/reunion et C/course")

    budget = os.getenv("GPI_BUDGET", "5")

    rc1 = run_cmd(["python","analyse_courses_du_jour_enrichie.py",
                   "--reunion",R,"--course",C,"--phase",when,"--budget",budget])

    rc2 = run_cmd(["python","validator_ev.py",
                   "--reunion",R,"--course",C,"--rules","gpi_v51.yml"])

    ok = (rc1 == 0 and rc2 == 0)
    return {"ok": ok, "R": R, "C": C, "phase": when, "rc_analyse": rc1, "rc_validator": rc2}
