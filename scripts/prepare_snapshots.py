...
def prepare_snapshot(csv_path: Path, race_reunion: int, race_course: int, output_dir: Path, output_filename: str):
    """
    ...
    with open(csv_path, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            if int(row['R']) == race_reunion and int(row['C']) == race_course:
                ...
                odds_val = row.get('rapportDirect') or row.get('coteDirect') or row.get('cote')
    ...
    print(f"Successfully created snapshot {output_path} for race R{race_reunion}C{race_course}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Prepare JSON snapshots from an odds CSV file.")
    parser.add_argument("--csv", required=True, help="Path to the input odds CSV file.")
    parser.add_argument("--reunion", required=True, type=int, help="Reunion number (e.g., 1).")
    parser.add_argument("--course", required=True, type=int, help="Course number (e.g., 6).")
    ...
    prepare_snapshot(Path(args.csv), args.reunion, args.course, Path(args.outdir), args.outfile)
