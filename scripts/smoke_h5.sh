...
URL_ARG=${1:-"https://www.zeturf.fr/fr/course/2025-10-11/R1C1-caen/"}
COURSE_URL=$URL_ARG
...
p_finale_path=$(find "$analysis_dir" -name '*p_finale.json' -print -quit)
if [ -z "$p_finale_path" ]; then
...
snapshot_path=$(find "$OUTPUT_DIR" -maxdepth 4 -name '*_H-5.json' -print -quit)
if [ -z "$snapshot_path" ]; then
  echo "[ERREUR] snapshot_H5.json introuvable" >&2
  exit 1
fi
grep -F '"phase": "H-5"' "$snapshot_path" >/dev/null || {
  echo "[ERREUR] snapshot_H5.json ne contient pas la phase H-5" >&2
  exit 1
}
