...
def fetch_program(d):
    return jget(f"{OFFLINE}/7/programme/{dmy(d)}")
    ...
    with csv_path.open("w", newline="", encoding="utf-8") as f:
        ...
        w.writerow(
            [
                ...
                "rapportProbable",
                "simpleGagnant_cote",
                "simpleGagnant_rapportDirect",
                "simpleGagnant_rapportProbable",
                "simplePlace_cote",
                "simplePlace_rapportDirect",
                "simplePlace_rapportProbable",
                "p_win_from_rapportDirect",
                "favori",
                "tendance",
            ]
        )
        for R, C, hippo, heure, disc in iter_fr_courses(program):
            ...
            for p in part.get("participants") or []:
                ...
                pwin = proba_from_rapport(
                    od.get("rapportDirect") or od.get("simpleGagnant_rapportDirect")
                )
                w.writerow(
                    [
                        d.isoformat(),
                        ...
                        od.get("coteDirect"),
                        od.get("coteProbable"),
                        od.get("coteReference"),
                        od.get("rapportDirect"),
                        od.get("rapportProbable"),
                        od.get("simpleGagnant_cote"),
                        od.get("simpleGagnant_rapportDirect"),
                        od.get("simpleGagnant_rapportProbable"),
                        od.get("simplePlace_cote"),
                        od.get("simplePlace_rapportDirect"),
                        od.get("simplePlace_rapportProbable"),
                        pwin,
                        fav,
                        trend,
                    ]
